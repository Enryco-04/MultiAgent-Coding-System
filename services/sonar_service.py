...
class SonarService:
    ...
    def scan(self, source_folder: str, iteration: int) -> None:
        """
        ...
        print(f"  [sonar] scanning {abs_folder} (token: {token_preview}) ...")
        ...
        self._wait_for_measures()

    # ── Wait for background task ──────────────────────────────────────────────

    def _wait_for_analysis(self, timeout: int = 60, poll_interval: int = 3) -> None:
        """
    ...
    def _wait_for_measures(self, timeout: int = 15, poll_interval: int = 1) -> None:
        """
    ...
    def get_issues(
        ...
        print(f"  [DEBUG][sonar] get_issues result_count={len(issues)}")
        return issues
